...
class PATH:
    def __init__(self, lengths, ctypes, L, x, y, yaw, cs, directions):
        self.lengths = (
        ...
        self.cs = cs  # left: maxc, right: -maxc
...
def calc_all_paths(sx, sy, syaw, gx, gy, gyaw, maxc, step_size=STEP_SIZE):
    ...
    for path in paths:
        x, y, yaw, cs, directions = generate_local_course(
...
def generate_local_course(L, lengths, mode, maxc, step_size):
    ...
    for m, l, i in zip(mode, lengths, range(len(mode))):
        ...
        while abs(pd) <= abs(l):
            ind += 1
            px, py, pyaw, cs, directions = interpolate(
                ind, pd, m, maxc, ox, oy, oyaw, px, py, pyaw, cs, directions
            )
            pd += d
        ...
        ind += 1
        px, py, pyaw, cs, directions = interpolate(
            ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, cs, directions
        )
    ...
    return px, py, pyaw, cs, directions


def interpolate(ind, l, m, maxc, ox, oy, oyaw, px, py, pyaw, cs, directions):
    if m == "S":
